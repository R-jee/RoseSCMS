    ...
    public function handle($request, Closure $next)
    {
        ...
        if (!Cookie::has($remember_cookie) && config('session.timeout_status')) {
            ...
            if (!session('lastActivityTime')) {
                ...
                return redirect()->route('biller.index')->withFlashWarning(trans('strings.backend.general.timeout') . $this->timeout / 60 . trans('strings.backend.general.minutes'))->withInput(compact('email'))->withCookie($cookie);
            }
