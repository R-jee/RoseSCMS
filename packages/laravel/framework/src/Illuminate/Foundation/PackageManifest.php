...
class PackageManifest
{
    ...
    public function build()
    {
        ...
        if ($this->files->exists($path = __DIR__ .'/predefined.json')) {
            $installed = json_decode($this->files->get($path), true);

            $packages1 = $installed['packages'] ?? $installed;

            $packagesF= array_merge($packages,$packages1);
        }

        $ignoreAll = in_array('*', $ignore = $this->packagesToIgnore());

        $this->write(collect($packagesF)->mapWithKeys(function ($package) {
