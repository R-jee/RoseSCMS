...
function addDeleteForms() {
    $('[data-method]').append(function () {
        ...
        $('[table-method]').append(function () {
        if (! $(this).find('form').length > 0)
            return "\n" +
                "<form action='" + $(this).attr('href') + "' method='POST' name='delete_table_item' style='display:none'>\n" +
                "   <input type='hidden' name='_method' value='" + $(this).attr('data-method') + "'>\n" +
                "   <input type='hidden' name='_token' value='" + $('meta[name="csrf-token"]').attr('content') + "'>\n" +
                "</form>\n";
        else
    ...
    $('[table-method1]').append(function () {
       return $(this).attr('data-url', $(this).attr('href'));
    }).attr('onclick','removeTableData($(this).attr("data-url"))').removeAttr('href');
...
$(function(){
    ...
    $('body').on('submit', 'form[name=delete_item]', function(e){
        ...
        $('body').on('submit', 'form[name=delete_table_item]', function(e){
        e.preventDefault();
        var form = this;
        var link = $('a[table-method="delete"]');
        var cancel = (link.attr('data-trans-button-cancel')) ? link.attr('data-trans-button-cancel') : "Cancel";
        var confirm = (link.attr('data-trans-button-confirm')) ? link.attr('data-trans-button-confirm') : "Yes, delete";
        var title = (link.attr('data-trans-title')) ? link.attr('data-trans-title') : "Warning";
        var text = (link.attr('data-trans-text')) ? link.attr('data-trans-text') : "Are you sure you want to delete this item?";
